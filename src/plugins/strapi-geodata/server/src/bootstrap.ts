    ...
    if (event.action === 'beforeCreate' || event.action === 'beforeUpdate') {
      for (const key in event.model.attributes) {
        ...
        if (field?.customField === 'plugin::geodata.geojson') {
          ...
          event.params.data.zoom = event.params.data[key]?.zoom ?? null;
          event.params.data.mapUrl = event.params.data[key]?.mapUrl ?? '';

  
          if (event.params.data.lat && event.params.data.lng) {
            event.params.data.geohash = geohash.encode(
              event.params.data.lat,
              event.params.data.lng,
              event.params.data.zoom ?? 12
            );
          }
        }
      }
